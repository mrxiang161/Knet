...
class ConvKernelHead(nn.Module):
    ...
    def __init__(self,
        ...
        if self.train_cfg:
            self.assigner = mmdet.core.build_assigner(self.train_cfg.assigner)
            # use PseudoSampler when sampling is False
            if self.sampling and hasattr(self.train_cfg, 'sampler'):
                sampler_cfg = self.train_cfg.sampler
            else:
                sampler_cfg = dict(type='MaskPseudoSampler')
            self.sampler = mmdet.core.build_sampler(sampler_cfg, context=self)
        self._init_layers()
    ...
    def loss(self,
        ...
        if cls_scores is not None:
            ...
            avg_factor = mmdet.core.reduce_mean(num_pos)
    ...
    def get_targets(self,
        ...
        if gt_sem_seg is None:
            gt_sem_seg = [None] * 2
            gt_sem_cls = [None] * 2
        results = mmdet.core.multi_apply(
